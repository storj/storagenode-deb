...
node {
	...
	stage('Test Repository') {
		...
		try {
			...
			sh "docker exec binaries-server zip /usr/share/nginx/html/storagenode_linux_amd64 . -i /usr/share/nginx/html/storagenode"
			sh "docker exec binaries-server zip /usr/share/nginx/html/storagenode-updater_linux_amd64 . -i /usr/share/nginx/html/storagenode-updater"
